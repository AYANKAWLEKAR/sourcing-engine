...
## Project Mode — Flat Upload (READ FIRST)

This workspace was uploaded to a Claude Project as a **flat set of files (no folders)**. These rules **override any folder paths mentioned later in this document** — treat those paths as illustrative and resolve everything by **filename**.

**Skills and rubrics** — each workflow's skill is `[Workflow]-SKILL.md` and its scoring rubric is `[Workflow]-SCORING-CRITERIA.xlsx`:

| Workflow | Skill file | Rubric file |
|---|---|---|
| Deal Sourcing | Deal-Sourcing-SKILL.md | Deal-Sourcing-SCORING-CRITERIA.xlsx |
| CIM Analysis | CIM-Analysis-SKILL.md | CIM-Analysis-SCORING-CRITERIA.xlsx |
| Commercial DD | Commercial-DD-SKILL.md | Commercial-DD-SCORING-CRITERIA.xlsx |
| Financial DD | Financial-DD-SKILL.md | Financial-DD-SCORING-CRITERIA.xlsx |
| Management DD | Management-DD-SKILL.md | Management-DD-SCORING-CRITERIA.xlsx |
| IC Memo | IC-Memo-SKILL.md | IC-Memo-SCORING-CRITERIA.xlsx |

Whenever a SKILL says "SCORING-CRITERIA.xlsx", use that workflow's rubric from the table above.

**Golden examples** (RevSpring) are uploaded as flat files — match by filename: `RevSpring_CIM_Screening_Report.docx`, `RevSpring_Financial_DD.pptx`, `RevSpring_Management_DD.pptx`, `RevSpring_Mgmt_DD_Interview_Guide.docx`, `RevSpring_IC_Memo.pptx`, `RevSpring_Financial_Model.xlsx`.

**Outputs** — a Project cannot write into folders. Produce each finished artifact as a **downloadable file** (.docx / .pptx / .xlsx) named per the SKILL, and tell the user where to file it. Any "create folders / save to disk" language below is for the Claude Code build and does not apply in a Project.
...
|---|---|
| CIM Analysis | `RevSpring_CIM_Screening_Report.docx` |
| Commercial DD | `RevSpring_CompetitiveAnalysis_2026-06-05.docx`, `RevSpring_MarketAnalysis_2026-06-05.docx` |
| Financial DD | `RevSpring_Financial_DD.pptx` |
| Management DD | `RevSpring_Management_DD.pptx` and `RevSpring_Mgmt_DD_Interview_Guide.docx` |
| IC Memo | `RevSpring_IC_Memo.pptx` |
...
| CIM Location | RevSpring_CIM.docx |
| Financial Model | RevSpring_Financial_Model.xlsx |
...
| Name | Role |
|---|---|
| Ayub | Principal, Morsebridge Healthcare (emailayankawlekar@gmail.com) |
| Scott MacKenzie | CEO, RevSpring | (ayan_kawlekar@berkeley.edu)
