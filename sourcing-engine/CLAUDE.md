...
python cli.py serve --ui                           # + Streamlit analyst UI on :8501 (GET / redirects to it)
...
- **`api/app.py`** — `create_app(manager)` FastAPI factory; 6 endpoints (`POST /runs`, `POST /runs/{id}/buybox`, `GET /runs/{id}`, `GET /runs/{id}/companies/{abn}[,/sources]`, `POST /runs/{id}/select`) plus `GET /` → redirect to `UI_URL` (the Streamlit UI). Handlers are plain `def` (blocking work → threadpool); serve with ONE worker only (in-process agent sessions).
- **`ui/`** (Part D) — Streamlit analyst UI. `api_client.py`: `OrigoClient`, a thin httpx wrapper over the 6 run endpoints (base URL from `ORIGO_API_URL`). `app.py`: single page, three phases via `st.session_state["phase"]` — `chat` (buy-box conversation) → `running` (stage-level `st.progress` bar polling `GET /runs/{id}`) → `results` (ranked shortlist + company-detail drawer with provenance). The `OrigoClient` is injectable via `st.session_state["client"]` so `AppTest` drives it with a fake. Streamlit runs its own server, so it's launched as a subprocess by `cli.py serve --ui` (not mounted in FastAPI); the API root just redirects to it. Tests: `tests/unit/test_ui_client.py` (respx), `tests/unit/test_ui_app.py` (AppTest).
